...
if getgenv().ChooseTeam == nil or getgenv().ChooseTeam == "" then getgenv().ChooseTeam = "Pirate" end
    ...
    if getgenv().AutoJoinTeam then
        if game:GetService("Players").LocalPlayer.PlayerGui:WaitForChild("Main"):FindFirstChild("ChooseTeam") then
            if game:GetService("Players").LocalPlayer.PlayerGui:WaitForChild("Main"):FindFirstChild("ChooseTeam") then
                ...
                Notify("RAM Account Status","getgenv().AutoJoinTeam = true --(joining team)",10)
            end 
        end
    else
        Notify("RAM Account Status","getgenv().AutoJoinTeam = false --(cant join team)",10)
        wait(10)
    end
until game.Players.LocalPlayer.Team ~= nil and game:IsLoaded()
wait(1)
Notify("RAM Account Status","Checking Account....",3)
...
function isinventoryontop()
    ...
    if getgenv().InventoryOnTop then
        ...
        cailonto = "Level: "..game.Players.LocalPlayer.Data.Level.Value.." | "..MeleeCount().."\nBeli: "..formatNumber(game.Players.LocalPlayer.Data.Beli.Value).."\nFragment: "..formatNumber(game.Players.LocalPlayer.Data.Fragments.Value).."\n"..CheckRace().."\nWorld: "..CheckWorld().."\nPlayers: "..PlayerCount().."\n"..CheckDevilFruit().."\nInventory: "..GetListWeapon(getgenv().InventoryRarity)..GetMastery(getgenv().InventoryRarity).."\nInventory Fruit: "..getInventoryFruits(getgenv().FruitInventoryRarity)
    end
    return cailonto;
end
...
if RAMPANEL then 
    while wait() do
        ...
        Notify("RAM Account Status","Set Description and Alias Success!",5)
        wait(30)
    end
end
